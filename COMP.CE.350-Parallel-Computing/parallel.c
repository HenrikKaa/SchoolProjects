...
// Implemented by user Zweedeend in https://stackoverflow.com/a/41637260
// Uncomment the next line to enable
//#define USE_FASTINVSQRT
inline double invsqrtQuake(double number)
{
	double y = number;
	double x2 = y * 0.5;
	int64_t i = *(int64_t *)&y;
...
void parallelPhysicsEngine()
{
		 ...
		 ++physicsUpdateIndex)
	{
		...
		for (int i = 0; i < SATELLITE_COUNT; ++i)
		{
			...
			// Use fast inverse square root instead
			double distToBlackHoleInverse = invsqrtQuake(distToBlackHoleSquared);
			// Changed to use inverse square root
			// Gravity force
			doublevector normalizedDirection = {
				.x = positionToBlackHole.x * distToBlackHoleInverse,
				.y = positionToBlackHole.y * distToBlackHoleInverse};
...
#pragma omp parallel for num_threads(4)
	for (int i = 0; i < SIZE; ++i)
	{
		...
		// This is not vectorised by gcc understandably because iterations depend on previous ones
		for (int j = 0; j < SATELLITE_COUNT; ++j)
		{
			floatvector difference = {.x = pixel.x - satellites[j].position.x,
									  .y = pixel.y - satellites[j].position.y};
			// Removed sqrt, not necessary for finding the closest satellite
			float distanceSquared = difference.x * difference.x + difference.y * difference.y;

			// Raised to second power to stay inline with removing sqrt
		...
		if (!hitsSatellite)
		{
			// This loop is 4/5th of the total computing time
			// This is not vectorised by gcc because first line not supported by stmt
			for (int j = 0; j < SATELLITE_COUNT; ++j)
			{
				floatvector difference = {.x = pixel.x - satellites[j].position.x,
										  .y = pixel.y - satellites[j].position.y};
