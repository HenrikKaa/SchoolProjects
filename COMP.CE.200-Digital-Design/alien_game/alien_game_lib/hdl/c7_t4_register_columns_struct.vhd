-- VHDL Entity alien_game_lib.c7_t4_register_columns.symbol
--
-- Created:
--          by - kaakkola.kaakkola (linux-desktop13.tuni.fi)
--          at - 20:36:48 12/05/20
--
...
ENTITY c7_t4_register_columns IS
   PORT( 
      clk      : IN     std_logic;
...
END c7_t4_register_columns ;

--
-- VHDL Architecture alien_game_lib.c7_t4_register_columns.struct
--
-- Created:
--          by - kaakkola.kaakkola (linux-desktop13.tuni.fi)
--          at - 20:36:48 12/05/20
...
ARCHITECTURE struct OF c7_t4_register_columns IS
   ...
   FOR ALL : c7_t4_register_row USE ENTITY alien_game_lib.c7_t4_register_row;
   -- pragma synthesis_on
...
BEGIN
   ...
   u_8combo_proc: PROCESS(dout, write, y)
   BEGIN
      CASE y(2) IS
      WHEN '0' => dout10 <= dout;
      WHEN '1' => dout10 <= write;
   ...
   u_11combo_proc: PROCESS(dout1, write, y)
   BEGIN
      CASE y(4) IS
      WHEN '0' => dout12 <= dout1;
      WHEN '1' => dout12 <= write;
   ...
   u_13combo_proc: PROCESS(dout2, write, y)
   BEGIN
      CASE y(5) IS
      WHEN '0' => dout13 <= dout2;
      WHEN '1' => dout13 <= write;
   ...
   u_15combo_proc: PROCESS(dout3, write, y)
   BEGIN
      CASE y(6) IS
      WHEN '0' => dout14 <= dout3;
      WHEN '1' => dout14 <= write;
   ...
   u_17combo_proc: PROCESS(dout4, write, y)
   BEGIN
      CASE y(7) IS
      WHEN '0' => dout15 <= dout4;
      WHEN '1' => dout15 <= write;
   ...
   u_19combo_proc: PROCESS(dout5, write, y)
   BEGIN
      CASE y(3) IS
      WHEN '0' => dout11 <= dout5;
      WHEN '1' => dout11 <= write;
   ...
   u_21combo_proc: PROCESS(dout6, write, y)
   BEGIN
      CASE y(1) IS
      WHEN '0' => dout8 <= dout6;
      WHEN '1' => dout8 <= write;
   ...
   u_23combo_proc: PROCESS(dout7, write, y)
   BEGIN
      CASE y(0) IS
      WHEN '0' => dout9 <= dout7;
      WHEN '1' => dout9 <= write;
   ...
   u_24combo_proc: PROCESS(pix_out, pix_out1, pix_out2, pix_out3, 
                           pix_out4, pix_out5, pix_out6, pix_out7, y)
   BEGIN
      dout16 <= (OTHERS => 'X');
      CASE y IS
   ...
   dout17 <= y(7) OR y(6) OR y(5) OR y(4) OR y(3) OR y(2) OR y(1) OR y(0);

   -- Instance port mappings.
   U_0 : c7_t4_register_row
      PORT MAP (
         clk      => clk,
         nullify  => nullify,
         pixd_in  => pixd_in,
