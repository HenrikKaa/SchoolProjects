...
void main() {
	light_dir = normalize(vec3(sin(uTime), -0.0, cos(uTime)));
	outColor = vec4(PBS(), 1.0);
}

// Returns normal using normal map
vec3 normal(){
	vec3 texture_normal = texture(normal_texture, textCoord.xy).xyz;
	vec3 bitangent = normalize(cross(vertexNormal, tangent));

	mat3 tbn = mat3(tangent, bitangent, vertexNormal);
	
	texture_normal = 2.0 * texture_normal - 1.0;
	vec3 normal = tbn * texture_normal;
...
vec3 PBS(){
	...
	float lcl = clamp(dot(normal(), light_dir), 0, 1);
...
float jsm(vec3 l, vec3 v, vec3 h){
	...
	float roughness = texture(roughness_texture, vertexPos.xy).x;
	float a2 = pow(roughness, 4);	// roughness
	float m_i = clamp(dot(normal(), l), 0, 1);
	float m_o = clamp(dot(normal(), v), 0, 1);
